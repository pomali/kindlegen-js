...
(function(){
  ...
  temp = require('temp');
  fs = require('fs');
  module.exports = function(epub, format, callback){
    temp.track();
    return temp.mkdir('node-kindlegen', function(error, tempDir){
      ...
      inputPath = path.join(tempDir, 'input.'+format);
      ...
      return fs.writeFile(inputPath, epub, function(error, written, string){
        ...
        kindlegen = spawn(path.resolve(__dirname, 'bin/kindlegen'), ['input.' + format, '-c2', '-verbose', '-o', 'output.mobi'], {
          cwd: tempDir,
          env: {}
        });
